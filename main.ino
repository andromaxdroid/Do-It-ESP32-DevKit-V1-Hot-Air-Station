/*
 * The "Do It ESP32 DevKit V1 Hot Air Station" project
 *
 * This project is an implementation of a hot air station using an ESP32 microcontroller.
 * It includes a temperature sensor, a display, and controls for adjusting the temperature.
 * lcd 16x2 i2c for displaying temperature and settings
...
const uint16_t temp_maxC = 450;               // Maximum possible temperature
...
const uint8_t ch_pins[3] = {CH1_pin, CH2_pin, CH3_pin}; // Array of channel pins for easier access
bool ch_last_state[3] = {HIGH, HIGH, HIGH};             // Array to store the state of each channel
unsigned long ch_press_time[3] = {0, 0, 0};             // Array to store the last press time for each channel
bool ch_saved[3] = {false, false, false};               // Array to track if the state of each channel has been saved to EEPROM
...
struct cfg
{
  ...
  uint16_t ch_temp[3];  // Temperatures for channels 1,2,3
...
void CONFIG::setChannel(uint8_t ch, uint16_t temp, uint8_t fan)
{
  if (ch < 3)
  {
...
void HOTGUN_CFG::init(void)
{
  CONFIG::init();
  if (!CONFIG::load())
    setDefaults(false); // If failed to load the data from EEPROM, initialize the config data with the default values
...
uint16_t HOTGUN_CFG::tempInternal(uint16_t t)
...
void HOTGUN_CFG::getChannel(uint8_t ch, uint16_t &temp, uint8_t &fan)
{
  CONFIG::getChannel(ch, temp, fan);
}

void HOTGUN_CFG::setChannel(uint8_t ch, uint16_t temp, uint8_t fan)
{
  CONFIG::setChannel(ch, temp, fan);
}
...
void HOTGUN_CFG::setDefaults(bool Write)
{
  ...
  for (uint8_t i = 0; i < 3; i++)
  {
    Config.ch_temp[i] = def_temp;
    Config.ch_fan[i] = def_fan;
  }
  if (Write)
  {
    CONFIG::save();
...
public:
  ...
  using LiquidCrystal_I2C::setCursor;
  ...
  void msgReady(void);
  void msgCold(void);
  void msgLoading(void); // Show message: "LOADING"
...
void DSPL::msgLoading(void)
{
  // Loading
  LiquidCrystal_I2C::setCursor(2, 0);
  LiquidCrystal_I2C::print(F("  LOADING"));
  LiquidCrystal_I2C::setCursor(0, 1);
  LiquidCrystal_I2C::print(F("[              ]"));
  for (uint8_t i = 0; i < 14; ++i)
  {
    LiquidCrystal_I2C::setCursor(1 + i, 1);
    LiquidCrystal_I2C::print((char)6);
    delay(100);
  }
  delay(1000);
  // Boot screen
  LiquidCrystal_I2C::clear();
  LiquidCrystal_I2C::setCursor(0, 0);
  LiquidCrystal_I2C::print(F("Hot Air Rework"));
  LiquidCrystal_I2C::setCursor(3, 1);
  LiquidCrystal_I2C::print(F("Booting..."));
  delay(2000);
  LiquidCrystal_I2C::clear();
  LiquidCrystal_I2C::setCursor(0, 0);
  LiquidCrystal_I2C::print(F("Project by;"));
  LiquidCrystal_I2C::setCursor(3, 1);
  LiquidCrystal_I2C::print(F("Thian Gani"));
  delay(2000);
}
...
//--------------------- High frequency PWM signal calss on D33
class FastPWM_D33
{
public:
  FastPWM_D33() {}
  ...
  void init(void)
  {
    ...
    const int pwmFreq = 25000;     // 25kHz (cocok untuk fan, tidak berisik)
    const int pwmResolution = 8;   // 0–255 (mirip OCR1A)

    ledcSetup(pwmChannel, pwmFreq, pwmResolution);
    ledcAttachPin(33, pwmChannel);

    _channel = pwmChannel;
    ledcWrite(_channel, 0); // off
  }

  void duty(uint8_t d)
  {
    ledcWrite(_channel, d); // set duty (0–255)
  }

  uint8_t fanSpeed(void)
  {
    return _lastDuty;
  }
...
void HOTGUN::switchPower(bool On)
{
  ...
  {
  case POWER_OFF:
    if (hg_fan.fanSpeed() == 0)
    {         // Not power supplied to the Fan
      if (On) // !FAN && On
        mode = POWER_ON;
    }
    else
...
void HOTGUN::keepTemp(void)
{
  ...
  if ((temp >= int_temp_max + 30) || (temp > (temp_set + 100)))
  { // Prevent global over heating
    if (mode == POWER_ON)
...
{
  if (Power == 0)
  { // To switch off the hot gun, set the Power to 0
    switchPower(false);
    return;
  }
...
void mainSCREEN::init(void)
{
  ...
  cool_notified = !used;
  if (used)
  { // the hot gun was used, we should save new data in EEPROM
...
{
  if (mode_temp)
  { // Prepare to adjust the fan speed
    uint8_t fs = pHG->getFanSpeed();
    pEnc->reset(fs, min_working_fan, 255, 5, 5);
    mode_temp = false;
  }
  else
  { // Prepare to adjust the preset temperature
    uint16_t temp_set = pHG->getTemp();
...
{ // Setup new preset temperature by rotating the encoder
  if (mode_temp)
  {
    ready = false;
...
SCREEN *calibSCREEN::show(void)
{
  ...
  if (tune && !pHG->isOn())
  { // The hot gun was switched off by error
    pD->msgOFF();
    tune = false;
    ready = false;
  }
...
void calibSCREEN::rotaryValue(int16_t value)
{
  update_screen = millis() + period;
  if (!tune)
  { // select the temperature to be calibrated, t_min, t_mid or t_max
    mode = value;
    if (mode > 2)
      mode = 2;
    uint16_t temp = temp_tip[mode];
...
{
  if (tune)
  { // Calibrated value for the temperature limit jus has been setup
    tune = false;
...
void calibSCREEN::buildCalibration(uint16_t tip[3])
{
  ...
  if (tip[2] > temp_max)
    tip[2] = temp_max; // Maximal possible temperature
}
...
SCREEN *pidSCREEN::menu(void)
{ // The encoder button pressed
  if (mode == 0)
...
void setup()
{
  ...
  pinMode(CH3_pin, INPUT_PULLUP); // Load configuration parameters
  hgCfg.init();
  hg.init();
  ...
  // Initialize channel button pins
  pinMode(CH1_pin, INPUT_PULLUP);
  pinMode(CH2_pin, INPUT_PULLUP);
...
void loop()
{
  ...
  for (uint8_t i = 0; i < 3; i++)
  {
    bool current_state = digitalRead(ch_pins[i]);
    if (current_state == LOW && ch_last_state[i] == HIGH)
    { // button just pressed
      ch_press_time[i] = millis();
      ch_saved[i] = false;
    }
    else if (current_state == LOW && !ch_saved[i] && millis() - ch_press_time[i] > 2000)
    { // still pressed and >2s, save immediately
      uint16_t current_temp = hg.getTemp();
      uint8_t current_fan = hg.getFanSpeed();
      hgCfg.setChannel(i, current_temp, current_fan);
      // Display message
      disp.clear();
      disp.setCursor(0, 0);
      disp.print("Saved to CH");
      disp.print(i + 1);
      delay(1000);
      pCurrentScreen->init(); // refresh screen
      ch_saved[i] = true;
    }
    else if (current_state == HIGH && ch_last_state[i] == LOW)
    { // button just released
      if (!ch_saved[i])
      { // short press, load
        uint16_t temp;
        uint8_t fan;
        hgCfg.getChannel(i, temp, fan);
        hg.setTemp(temp);
        hg.setFanSpeed(fan);
        // Display message
        disp.clear();
    ...
    ch_last_state[i] = current_state;
  }

  nxt = pCurrentScreen->show();
  if (nxt && pCurrentScreen != nxt)
  { // Be paranoiac, the returned value must not be null
    pCurrentScreen = nxt;
    pCurrentScreen->init();
    reset_encoder = true;
  }
